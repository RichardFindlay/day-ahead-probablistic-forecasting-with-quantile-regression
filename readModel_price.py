...
times_out = np.empty_like(f['train_set']['X2_train'][0])
...
time_set_load = open(f"./Data/{type}/Processed_Data/time_refs_V1_DAprice.pkl", "rb") # demand
...
X_train3 = f[f'{set_type}_set'][f'X2_{set_type}'][0:4500]
...
while (output_start + output_seq_size) <= len(y_train):
	...
	# outputs
	seqX3.append(X_train2[output_start:output_end])
...
predictions = model.predict([x1, x2, x3, s0, c0])
predictions1 = model1.predict([x1, x2, x3, s0, c0])
predictions2 = model2.predict([x1, x2, x3, s0, c0])
