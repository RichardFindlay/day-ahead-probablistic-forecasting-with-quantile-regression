...
# load main model
model = load_model(f'../../models/seq2seq+temporal+spatial/{model_type}/{model_type}_main.h5', custom_objects = {'<lambda>': lambda y,f: defined_loss(q,y,f), 'attention': attention, 'Activation': Activation(swish)})

# read encoder models - igoring the spatail encoder in the price forecasting case
temporal_enc = load_model(f'../../models/seq2seq+temporal+spatial/{model_type}/{model_type}_temporal_enc.h5') 

if model_type != "price":
	spatial_enc = load_model(f'../../models/seq2seq+temporal+spatial/{model_type}/{model_type}_spatial_enc.h5') 
...
def inference_dec_model(quantile):
	...
	decoder_input_with_prev = concatenate([decoder_input, prev_prediction])
...
with open(f'../../results/seq2seq+temporal+spatial/{model_type}/forecasted_time_series_{model_type}.pkl', 'wb') as ts_file:
	dump(predictions, ts_file)

# save results - forecasted temporal attention matrix
with open(f'../../results/seq2seq+temporal+spatial/{model_type}/attention_data_{model_type}.pkl', 'wb') as attention_file:
	dump(quantile_temporal_attns, attention_file)

# save results - forecasted spatial attention matrix
with open(f'../../results/seq2seq+temporal+spatial/{model_type}/attention_data_{model_type}.pkl', 'wb') as spatial_file:
