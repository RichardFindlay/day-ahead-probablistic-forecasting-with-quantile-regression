...
class DataGenerator(tensorflow.keras.utils.Sequence):
	...
	def __getitem__(self, index):
		...
		return (X_train1, X_train2, X_train3, X_train4), (y_trues) # pass empty training outputs to extract extract attentions
	...
	def __data_generation(self, input_indexes, output_indexes):
		...
		if model_type != 'price':        
			...
			X_train4 = f['train_set']['X1_train'][output_indexes][:,:,:,:]
...
for q in quantiles: 
	...
	encoder_output, forward_h, forward_c, backward_h, backward_c = Bidirectional(LSTM(32, return_sequences = False, return_state = True), name=f'biLSTM_q_{q}')(combined_inputs)
	repeat_layer = RepeatVector(48)(encoder_output)

	enc_h = concatenate([forward_h, backward_h])
	enc_s = concatenate([backward_h, backward_h])

	decoder_input = concatenate([repeat_layer, times_out])
	decoder_input = concatenate([decoder_input, out_nwp])

	decoder_out, _, _  = LSTM(64, return_sequences = True, return_state = True, name=f'decoder_LSTM_q_{q}')(decoder_input, initial_state = [enc_h, enc_s])

	dense_out = TimeDistributed(Dense(1, name=f'dense_q_{q}'))(decoder_out)

	if model_type == 'solar':
		dense_out = Activation('relu', name=f'relu_act_q_{q}')(dense_out)

	quantile_predictions.append(dense_out)

model = Model(inputs = [x_input, times_in, times_out, out_nwp], outputs = quantile_predictions)
...
model.save(f'../../models/{model_type}/{model_type}_seq2seq.h5')
