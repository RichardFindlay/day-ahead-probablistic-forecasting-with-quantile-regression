...
# get size parameters
f = h5py.File(f"/content/drive/My Drive/new_data/{model_type}/{dataset_name}", "r")
...
class DataGenerator(tensorflow.keras.utils.Sequence):
	...
	def __data_generation(self, input_indexes, output_indexes):
		...
		f = h5py.File(f"/content/drive/My Drive/new_data/{model_type}/{self.dataset_name}", "r")      
...
for q in quantiles: 
	...
	output_2 = Dense(1, name=f'dense2_q_{q}')
	...
	for ts in range(Ty):
		...
		# decoder_input = concatenate([context, decoder_input], axis=-1, name=f'concat5_q_{q}_{ts}')
...
b = lambda y,f: defined_loss(0.15,y,f) 
c = lambda y,f: defined_loss(0.25,y,f)
d = lambda y,f: defined_loss(0.35,y,f)
e = lambda y,f: defined_loss(0.5,y,f) 
f = lambda y,f: defined_loss(0.65,y,f)
g = lambda y,f: defined_loss(0.75,y,f)
h = lambda y,f: defined_loss(0.85,y,f) 
i = lambda y,f: defined_loss(0.95,y,f)


listOfLambdas = [lambda y,f: K.mean(K.maximum(q*(y - f), (q-1) * (y - f)), axis = -1) for q in quantiles]

q_losses = [a, b, c, d, e, f, g, h, i]
...
train = model.fit(training_generator, epochs = 15)

os.mkdir(f'/content/drive/My Drive/{model_type}Models/q_{q}_seq2seq+temporal')
# model_freeze.save_weights('/content/drive/My Drive/solarGeneration_forecast_weights_freezed' + '_Q_%s' %(q) + '.h5')
model.save(f'/content/drive/My Drive/{model_type}Models/q_{q}_seq2seq+temporal/{model_type}_seq2seq+temporal_main.h5')
...
enoder_temporal_model.save(f'/content/drive/My Drive/{model_type}Models/q_{q}_seq2seq+temporal/{model_type}_seq2seq+temporal_enc.h5')
