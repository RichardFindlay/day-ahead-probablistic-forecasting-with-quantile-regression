...
if model_type == 'wind':
	dataset_name = 'train_set_V8_avglabels.hdf5'
...
# encoder layers
lstm_encoder = LSTM(n_s, return_sequences = False, return_state = True, stateful=True)
...
lstm_decoder = LSTM(n_s, return_sequences = True, return_state = True, batch_input_shape=(params['batch_size'], Tx, times_in_dim + channels), stateful=True)
...
# define inputs
x_input = Input(batch_shape=(params['batch_size'], Tx, height, width, channels))
times_in = Input(batch_shape=(params['batch_size'], Tx, times_in_dim))
times_out = Input(batch_shape=(params['batch_size'], Ty, times_out_dim))
out_nwp = Input(batch_shape=(params['batch_size'], Ty, channels-1))
s_state0 = Input(batch_shape=(params['batch_size'], n_s,))
c_state0 = Input(batch_shape=(params['batch_size'], n_s,))
...
for q in quantiles:
	...
	for e in range(EPOCHS):
		model.fit(training_generator, workers=4, epochs = 3)
		model.reset_states()
	...
	# save some additional models for inference
	encoder_model_temporal = Model(inputs = [x_input, times_in], outputs=[lstm_enc_output, enc_s_state, enc_c_state]) 
	...
	encoder_model_temporal.save(f'/content/drive/My Drive/{model_type}Models/q_{q}/{model_type}Generation_encoderModel_temporal' + '_Q_%s' %(q) + '.h5')
	...
	enoder_spatial_model.save(f'/content/drive/My Drive/{model_type}Models/q_{q}/{model_type}Generation_encoderModel_spatial' + '_Q_%s' %(q) + '.h5')
