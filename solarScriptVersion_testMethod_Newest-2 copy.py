...
for q in quantiles: 
	...
	decoder = LSTM(32, return_sequences = True, return_state = True, name=f'decoder_q_{q}')
	...
	for ts in range(Ty):
		...
		decoder_input = concatenate([decoder_input, context], axis=-1, name=f"concat4_q_{q}_{ts}") 
		...
		dec_output, s_state, c_state = decoder_2(dec_output, initial_state = [s_state, c_state])
		...
		# output = Dense(64, activation="relu", name=f'dense1_q_{q}_{ts}')(prediction)
		# prediction = BatchNormalization()(prediction)  
		# output = Dense(32, activation="tanh", name=f'dense1_q_{q}_{ts}')(prediction)
		# output = concatenate([prev_prediction, output])
		...
		output = Dense(1, name=f'dense2_q_{q}_{ts}')(dec_output)
	...
	# ts_predictions_total = TimeDistributed(Dense(1, name=f'dense2_q_{q}_{ts}'))(ts_predictions_total)
...
train = model.fit(training_generator, epochs = 20)
